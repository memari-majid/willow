...
export type WillowContent = {
  ...
  /** Sokol & Fox–grounded CBT companion protocol (see build spec). */
  cbtCompanionInstructions: string;
  /** Warm-competent voice calibration — anti-patterns and worked examples. */
  cbtCompanionToneAndPersona: string;
...
export async function loadContent(): Promise<WillowContent> {
  ...
  const [
    ...
    references,
    glossary,
    cbtCompanionInstructions,
    cbtCompanionToneAndPersona,
    techniques,
    checkIns,
  ] = await Promise.all([
    reads.persona,
    ...
    reads.references,
    reads.glossary,
    readMdOptional("cbt_companion_instructions.md"),
    readMdOptional("cbt_companion_tone_and_persona.md"),
    loadTechniques(),
    loadCheckIns(),
  ]);
  ...
  const status: FileStatus[] = [
    ...
    statusOf("cbt_companion_instructions.md", cbtCompanionInstructions),
    statusOf("cbt_companion_tone_and_persona.md", cbtCompanionToneAndPersona),
    ...techniques.map((t) =>
...
async function readMdOptional(relativePath: string): Promise<string> {
  try {
    return await readMd(relativePath);
  } catch {
    return "";
  }
}
